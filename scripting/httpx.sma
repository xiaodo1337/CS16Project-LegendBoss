...
public plugin_init() {
	...
	g_FailedForward = CreateMultiForward("HTTPX_Connect_Failed", ET_IGNORE, FP_CELL)
...
Download(const URL[], const Filename[], const CompleteHandler[], const ProgressHandler[], Port, RequestType, const Username[], const Password[], PluginID, DownloadID, CustomValue = 0) {
	...
	if ( Temp ) {
		...
		ExecuteForward(g_FailedForward, gReturnDummy, i)
		gPostVars[0] = 0;
		return;
	}
...
FollowLocation(Index, const Location[]) {
	...
	if ( ResultNum ) {
		...
		ExecuteForward(g_FailedForward, gReturnDummy, Index)
		return 0;
	}
